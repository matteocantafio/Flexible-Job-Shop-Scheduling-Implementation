...
# Efficiency parameters for each machine
...
for _ in range(m):
    ...
    while True:
        ...
        alfa = input("Insert efficiency parameter: ")
        ...
        try:
            ...
            print("Error: alpha must be float type 0.x.")

# Calculate total efficiency

total_efficiency = 1 - np.prod([1 - eff for eff in efficiency_params])
...
for i in range(n):
    for j in range(1, 2):  # Assuming each job has two operations
        model.add_constraint(sum(S[i, j, k] for k in Mj[i, j]) >= sum(C[i, j - 1, k] for k in Mj[i, j]),
                             ctname=f'precedence_{i}_{j}')
...
for i in range(n):
    for j in range(2):  # Assuming each job has two operations
        ...
        for k in Mj[i, j]:
            ...
            model.add_constraint(C[i, j, k] >= S[i, j, k] + t[i, j, k] / efficiency_params[k] - (1 - X[i, j, k]) * L,
                                 ctname=f'completion_{i}_{j}_{k}')
            for i_prime in range(n):
                if i != i_prime:
                    for j_prime in range(2):
                        if k in Mj[i_prime, j_prime]:
                            model.add_constraint(
                                S[i, j, k] >= C[i_prime, j_prime, k] - Y[i, j, i_prime, j_prime, k] * L,
                                ctname=f'precedence_{i}_{j}_{i_prime}_{j_prime}_{k}')
                            model.add_constraint(
                                S[i_prime, j_prime, k] >= C[i, j, k] - (1 - Y[i, j, i_prime, j_prime, k]) * L,
                                ctname=f'precedence_{i_prime}_{j_prime}_{i}_{j}_{k}')
                ...
                for i_prime in range(n):
                    ...
                    if i != i_prime:
                        ...
                        for j_prime in range(2):
                            ...
                            if k in Mj[i_prime, j_prime]:
                                print(f"Y_{i}_{j}_{i_prime}_{j_prime}_{k}:",

                                      solution.get_value(Y[i, j, i_prime, j_prime, k]))
